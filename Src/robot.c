...
static uint32_t robot_convert_encoder_data(robot_t * robot, currLeftData, currRightData, prevLeftData, prevRightData);
...
static state_method_t state_methods[NUM_STATES] =
{
	...
	(state_method_t) sm_turning_right, (state_method_t) sm_turn_around,
	(state_method_t) sm_solving, (state_method_t) sm_solving_complete,
	(state_method_t) sm_stop, (state_method_t) sm_racing,
	(state_method_t) sm_measure, (state_method_t) sm_dead_reckoning
};
...
{
	(*state_methods[robot->current_state])(robot->current_state);
}
...
static void sm_measure(robot_t *robot)
{
	/**
